...
| `/roles hide / show` | Remove/repost the whole opt-in role message |
...
| `/board time-format / time-reset / time-list` | Customize the board's "6m left"/"in 1h" wording |
...
Defaults: preset buttons = Everyone. `setup`, `language`, `board` (covers `/roles hide|show` too) = Manage Server — these change how the bot presents to the *whole server* (layout, language, wording, or the opt-in message's presence). Everything else = Manage Messages. `/permissions` itself is hardcoded Manage Server.
...
- Button/label/wording changes need a repost (`/setup` or `/roles message`) to show on a new message.
- Russian pings and board wording ("осталось {time}" / "через {time}") are built-in defaults when `/language set` is Russian — override either per server with `/pings message` / `/board time-format`.
